...
namespace Drupal\curso_content\Plugin\rest\resource;
 ...
 *
 * @RestResource (
 *   id = "curso_content_example",
 *   label = @Translation("Example"),
 *   uri_paths = {
 *     "canonical" = "/api/curso-content-example/{id}/{id2}/{id3}",
 *     "https://www.drupal.org/link-relations/create" = "/api/curso-content-example"
 *   }
 * )
 *
 * @DCG
...
class ExampleResource extends ResourceBase implements DependentPluginInterface {
   ...
   */
  public function get($id, $status, $asas) {
  ...
  public function post($data) {
    ...
    $id = $this->dbConnection->insert('curso_content_example')
      ->fields($data)
      ->execute();

    $this->logger->notice('New example record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('curso_content_example')
      ->condition('id', $id)
      ->execute();
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {curso_content_example} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('curso_content_example')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('Example record @id has been updated.', ['@id' => $id]);
